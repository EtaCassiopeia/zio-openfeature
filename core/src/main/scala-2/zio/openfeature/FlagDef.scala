...
/** A typed flag definition: key, type and default value as a single first-class value.
  ...
  * site rather than reusing this instance. That is invisible for the built-in types (their instances are unique), but
  * a `FlagDef` built with an explicitly-passed instance will pick up whatever instance is in scope at the `copy` — so
  ...
  */
case class FlagDef[A](key: String, default: A, description: String = "")(implicit val flagType: FlagType[A]) {

  /** True when `other` names the same flag key, regardless of its type parameter or default. */
  def sameKey(other: FlagDef[_]): Boolean = key == other.key
}
