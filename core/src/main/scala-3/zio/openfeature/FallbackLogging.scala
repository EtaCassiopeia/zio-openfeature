...
/** How the total tier (`*OrDefault` / `resolveOrDefault`) logs a served-default fallback — the built-in warn line that
  ...
  *
  * An absorbed *defect* is a bug, not outage noise: its line has its own per-key bucket under `Throttled` and is still
  * emitted under `Off` — this breadcrumb is the only place a swallowed bug surfaces from the value-only `*OrDefault`
  * variants.
  */
